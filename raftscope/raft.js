...
var raft = {};
var RPC_TIMEOUT = 500000;
var MIN_RPC_LATENCY = 0;
var MAX_RPC_LATENCY = 0;
var ELECTION_TIMEOUT = 50000;
var NUM_SERVERS = 8;
var BATCH_SIZE = 1;
var NUM;
var NUM_RAND;
...
rules.startNewElection = function(model, server) {
  ...
  incre++;
  if ((server.electionAlarm <= model.time) && ((incre%NUM_SERVERS)==2)) {

    NUM = Math.random();
    NUM_RAND = NUM*NUM_SERVERS;
    console.log(NUM_RAND);
  }

  if ((server.state == 'follower' || server.state == 'candidate' || server.state == 'leader') &&
  ...
  //  if ((NUM < .3333333) && (server.id == 1)) {
  //    server.state == 'leader';
  //    console.log('leader 1 is elected!');
    ...
    //server.rpcDue       = util.makeMap(server.peers, 0);
    //server.heartbeatDue = util.makeMap(server.peers, 0);
  //} else if ((NUM < .666666) && (server.id == 2) & (NUM > .333333)) {
    
  //  server.state == 'leader';

  //  console.log('leader 2 is elected!');


  //} else if ((NUM > .666666) && (server.id == 3)) {
  //  server.state == 'leader';

  //  console.log('leader 3 is elected!');
  //  }
  }
};
...
rules.becomeLeader = function(model, server) {
  if (server.state == 'candidate' && (server.id - NUM_RAND) <1 && (server.id - NUM_RAND) >0) {

    server.state = 'leader';
    console.log('leader ' + server.id + ' is elected!');
